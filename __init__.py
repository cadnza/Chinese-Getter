...
def insert_char(note):
	if isEmpty(f_char,note) and not isEmpty(f_pinyin,note):
		...
		try:
			...
			charURL = wiktBase + pinyinHTML.select("a[href*=\"#Chinese\"]")[0].get("href")
...
def insert_sound(note):
	if isEmpty(f_sound,note) and not isEmpty(f_char,note) and len(forvoKey) > 0:
		...
		forvoURL = "https://apifree.forvo.com/action/word-pronunciations/format/xml/word/" + chars + "/language/zh/order/rate-desc/limit/1/key/" + forvoKey + "/"
